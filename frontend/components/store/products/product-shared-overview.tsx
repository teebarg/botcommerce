...
const ProductSharedOverview: React.FC<{
    product: Product;
    isLiked?: boolean;
    onClose: () => void;
    ...
    useEffect(() => {
        if (user && product?.id) {
            trackInteraction.mutate({
                ...
                type: "VIEW",
                metadata: { source: "product-shared-overview" },
            });
        }
        ...
        return () => {
            ...
            if (user && product?.id) {
                trackInteraction.mutate({
                    ...
                    product_id: product.id,
                    type: "VIEW",
                    metadata: { timeSpent, source: "product-shared-overview" },
                });
            }
        };
    ...
    const selectedImage = product.images[selectedImageIdx]?.image;
        ...
        <div className="bg-content1 z-50 overflow-y-auto duration-300 w-full rounded-[inherit]">
            ...
            <div className="flex flex-wrap gap-4 px-2 mt-4">
                {product.images.map((image: ProductImage, idx: number) => (
                    ...
                    >
                        <Image fill alt={`Thumbnail - ${image}`} className="object-cover" sizes="64px" src={image.image} />
                    </button>
                ))}
            </div>
...
export default ProductSharedOverview;
