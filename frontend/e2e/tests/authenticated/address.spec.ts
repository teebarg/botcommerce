...
test.describe("Addresses tests", () => {
    test("Creating a new address is displayed during checkout", async ({
        ...
        await test.step("Verify the address is correct in the checkout process", async () => {
            ...
            expect(await getSelectedOptionText(checkoutPage.page, checkoutPage.shippingCountrySelect)).toContain("United States");
        });
    });

    test("Performing all the CRUD actions for an address", async ({ accountAddressesPage: addressesPage }) => {
        await test.step("Navigate to the new address modal", async () => {
            await addressesPage.goto();
            await addressesPage.newAddressButton.click();
            await addressesPage.addAddressModal.container.waitFor({ state: "visible" });
    ...
    test.skip("Attempt to create duplicate addresses on the address page", async ({ accountAddressesPage: addressesPage }) => {
        await test.step("navigate to the new address modal", async () => {
            await addressesPage.goto();
            await addressesPage.newAddressButton.click();
            await addressesPage.addAddressModal.container.waitFor({ state: "visible" });
    ...
    test("Creating multiple tests works correctly", async ({ accountAddressesPage: addressesPage }) => {
        test.slow();
        await test.step("Navigate to the new address modal", async () => {
            await addressesPage.goto();
        });
