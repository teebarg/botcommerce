...
export function Pagination({ page, totalPages, "data-testid": dataTestid }: { page: number; totalPages: number; "data-testid"?: string }) {
    ...
    const renderPageButton = (p: number, label: string | number, isCurrent: boolean) => (
        <button
            key={p}
            className={clsx("text-xl text-default-500", {
                "text-default-800 hover:text-default-500": isCurrent,
            })}
            disabled={isCurrent}
    ...
    const renderEllipsis = (key: string) => (
        <span key={key} className="text-xl text-default-500 items-center cursor-default">
            ...
        </span>
    );
