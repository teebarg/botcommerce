...
export async function POST(req: NextRequest) {
    try {
        const { to, name, email, message, phone } = await req.json();
        ...
        const templatePath = path.resolve("./src/email-templates/contact.hbs");
        ...
        const mailOptions = {
            ...
            html: template({ name, email, message, phone, projectName: "TBO" }), // Compile the template with dynamic data
        };
