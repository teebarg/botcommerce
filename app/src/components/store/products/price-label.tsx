...
export const PriceLabel: React.FC<PriceLabelProps> = ({
    ...
    if (minPrice === maxPrice) {
            ...
            return (
                <div>
                    <div className="flex items-center space-x-2">
                        ...
                        <span className={cn("line-through text-xs truncate", oldPriceClassName)}>{currency(maxCompareAtPrice!)}</span>
                    </div>
                    {/* {hasDiscount && (
        ...
        return <div className={cn("text-xl font-bold", className, priceClassName)}>{currency(minPrice)}</div>;
    }
