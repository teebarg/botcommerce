...
def get_cart_service(settings_service: ShopSettingsService = Depends(get_shop_settings_service), coupon_srv: CouponService = Depends(get_coupon_service)) -> CartService:
    return CartService(
        db=db,
