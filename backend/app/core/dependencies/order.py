...
def get_order_service(settings_service: SettingsDep, evt_bus: EventBus = Depends(get_event_bus), coupon_srv: CouponService = Depends(get_coupon_service)) -> OrderService:
    return OrderService(
        db=db,
