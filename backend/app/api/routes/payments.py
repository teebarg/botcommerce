...
async def initialize_payment(
    order_id: int, srv: OrderDep, payment_srv: PaymentDep, user: CurrentUser
):
...
async def paystack_webhook(
    request: Request, payment_srv: PaymentDep, x_paystack_signature: str = Header(...)
):
...
async def change_payment_status(
    db: DbDep, srv: OrderDep, id: int, status: PaymentStatus, payment_srv: PaymentDep
) -> Order:
    """Change payment status"""
    order = await db.order.find_unique(
        where={"id": id}, include={"order_items": {"include": {"variant": True}}, "payment": True}
    )
    if not order:
        raise HTTPException(status_code=404, detail="order not found")
    ...
    for item in order.order_items:
        if item.variant and item.variant.inventory < item.quantity:
            raise HTTPException(
                status_code=400,
                detail="order has out of stock items, cannot update payment status",
            )
    ...
    if status == PaymentStatus.SUCCESS:
        ...
        await payment_srv.record_success(reference=reference)
        return order
